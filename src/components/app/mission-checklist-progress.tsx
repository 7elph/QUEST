  ...
  return (
    <section className="rounded-xl border border-amber-200/20 bg-black/20 p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-lg font-semibold">Andamento da Missao</h2>
        <span className="rounded-full border border-amber-200/30 bg-amber-500/10 px-2 py-1 text-xs text-amber-200">
          {pct}% concluido
        </span>
      </div>
      ...
      <div className="mt-4 space-y-2">
        {conditions.map((item, idx) => (
          <label key={`${idx}-${item}`} className="flex items-start gap-2 rounded-md border border-amber-100/10 bg-black/25 p-2 text-sm">
            <input
              type="checkbox"
      ...
      {!canEdit && <p className="mt-2 text-xs text-amber-100/75">Somente o aventureiro atribuido pode marcar itens como concluidos.</p>}
      {updatedAt && <p className="mt-1 text-xs text-amber-100/70">Ultima atualizacao: {new Date(updatedAt).toLocaleString("pt-BR")}</p>}
